...
import './ExperimentManagerUI.css';
import { createExperimentCall, updateExperimentCall, deleteExperimentCall } from "./backend_modules/services/ExperimentsService";
import { fetchEncoders } from './api'; // Import your API function to fetch encoders
...
export default function ExperimentManagerUI() {
    ...
    const [selectedEncoders, setSelectedEncoders] = useState([]);
    const [activeTab, setActiveTab] = useState('create');
    const [encoders, setEncoders] = useState([]); // State to hold encoder data
    ...
    useEffect(() => {
        const loadEncoders = async () => {
            try {
                ...
                setEncoders(encoderData); // Update state with fetched encoder data
            } catch (error) {
    ...
    const handleEncoderToggle = (encoder) => {
        ...
        // Update form data based on selected encoder
        const selectedEncoderData = encoders.find(enc => enc.name === encoder);
        if (selectedEncoderData) {
            setFormData((prev) => ({
                ...prev,
                Encoder: selectedEncoderData.name,
                Bitrate: selectedEncoderData.bitrate, // Example field
                // Add other fields as necessary
            }));
        }
    };
                        ...
                        <>
                            <div className="ui-form-section">
                                ...
                                <div className="ui-form">
                                    ...
                                    {!useJsonConfig && (
                                        <div className="ui-select-grid">
                                            {/* Your existing select fields */}
                                            {/* Example select fields can be added here */}
                                        </div>
                                    )}
                                    ...
                                    <div className="ui-select-grid">
                                        <label className="ui-label">
                                            ...
                                            >
                                                ...
                                                <Select.Content>
                                                    <Select.Viewport>
                                                        {/* Populate QP options based on selected encoder */}
